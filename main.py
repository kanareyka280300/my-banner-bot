...
intents.bans = True      # ДОЗВОЛЯЄ БОТУ БАЧИТИ БАНИ/РОЗБАНИ
intents.moderation = True  # ДОЗВОЛЯЄ БАЧИТИ ТАЙМАУТИ (МУТИ)
...
SECURITY_LOG_CHANNEL_ID = 1524853896822915173 # Канал для звітів про інвайти та анти-твінків (запасний варіант)

# --- НОВІ КАНАЛИ ЛОГУВАННЯ ("папки") ---
# Заміни кожне значення на реальний ID потрібного каналу.
# Якщо лишиш SECURITY_LOG_CHANNEL_ID - всі логи підуть в один канал.
BAN_LOG_CHANNEL_ID = 1489741516971966655        # 1. папка "бан"
JOIN_LEAVE_LOG_CHANNEL_ID = 1524853896822915173 # 2. папка "системні: зайшов/вийшов" + повна анкета
...
GENERAL_LOG_CHANNEL_ID = 1489742637278822531    # 7. папка "сервер загальне" (мут, створення войсу тощо)
# =========================================================================
...
async def get_audit_executor(guild, action, target_id=None, within_seconds=10):
    """Шукає в audit log останній запис потрібної дії за останні N секунд,
    опційно відфільтрований по цілі (target_id). Повертає executor або None."""
    try:
...
async def on_member_join(member):
    guild = member.guild
    security_channel = bot.get_channel(JOIN_LEAVE_LOG_CHANNEL_ID)
    ...
    if stored:
        embed.add_field(
            ...
            value=(f"• Зайшов: `{stored.get('joined_at')}`\n"
                   ...
                   f"• Хто запросив: {f'<@{stored.get(\"inviter_id\")}> (`{stored.get(\"inviter_name\")}`)' if stored.get('inviter_id') else 'Невідомо'}\n"
...
async def on_member_ban(guild, user):
    ...
    moderator_text = "Невідомо"
    reason_text = "Причина не вказана"
    if entry:
        moderator_text = f"{entry.user.mention} (`{entry.user.name}`)"
        if entry.reason:
            reason_text = entry.reason

    embed = discord.Embed(
        title="🔨 НОВИЙ БАН",
        description=f"{user.mention} (`{user.name}`, ID: `{user.id}`)",
        color=0xff0000
    )
...
# =========================================================================
# 3 і 4. РОЛІ ТА НІКНЕЙМИ (+ GTA рекрутинг, як і було)
# =========================================================================
@bot.event
async def on_member_update(before, after):
    guild = after.guild

    # --- GTA рекрутинг (без змін) ---
    ...
    # --- ЛОГ НІКНЕЙМІВ (тільки поки людина на сервері) ---
    ...
    if nick_channel and before.nick != after.nick:
        ...
        embed = discord.Embed(
            ...
            description=f"{after.mention} (`{after.name}`, ID: `{after.id}`)",
            color=0xffff00
        )
...
# =========================================================================
# 5. ПОВІДОМЛЕННЯ (редагування / видалення)
# =========================================================================
@bot.event
async def on_message_delete(message):
    ...
    if entry and entry.user.id != message.author.id:
        deleter_text = f"{entry.user.mention} (`{entry.user.name}`)"
    else:
        deleter_text = f"{message.author.mention} (сам(а) видалив(ла))"
    ...
    content = message.content if message.content else "*(без тексту / вкладення)*"
...
async def on_message_edit(before, after):
    ...
    embed.add_field(name="Дата/час:", value=now_str(), inline=False)
    if before.jump_url:
        embed.add_field(name="Посилання:", value=before.jump_url, inline=False)
    await channel.send(embed=embed)
...
async def on_voice_state_update(member, before, after):
    ...
    if before.channel is None and after.channel is not None:
        embed = discord.Embed(
            title="🎙️ ЗАЙШОВ У ВОЙС",
            description=f"{member.mention} (`{member.name}`) → {after.channel.mention}",
            color=0x00ff7f
        )
        embed.add_field(name="Дата/час:", value=now_str(), inline=False)
        await channel.send(embed=embed)
    ...
    if before.channel is not None and after.channel is None:
        ...
        embed = discord.Embed(
            title="🔇 ВИЙШОВ З ВОЙСУ",
            description=f"{member.mention} (`{member.name}`) ← {before.channel.mention}",
            color=0xff6347
        )
    ...
    # перейшов з одного войсу в інший
    if before.channel is not None and after.channel is not None and before.channel.id != after.channel.id:
        entry = await get_audit_executor(member.guild, discord.AuditLogAction.member_move, target_id=member.id)
        if entry:
            by_text = f"Переміщений(а) модератором {entry.user.mention} (`{entry.user.name}`)"
        else:
            by_text = "Перейшов(ла) самостійно"

        embed = discord.Embed(
            title="🔀 ПЕРЕМІЩЕННЯ У ВОЙСІ",
...
# =========================================================================
# 7. СЕРВЕР ЗАГАЛЬНЕ (створення каналів тощо)
# =========================================================================
@bot.event
